...
describe('Users Query - Find By Email', () => {
  ...
  it('Should not return user if id does not exist', async () => {
    ...
    await expect(query.findByEmail('123')).rejects.toThrowError(
      new NotFoundException(`User not found with email:123`)
    )
  })
  ...
  it('Should return user with same id', async () => {
    ...
    expect(await query.findByEmail(user.email)).toEqual(
